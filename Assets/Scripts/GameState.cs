...
public sealed class GameState : NetworkBehaviour, ISpawned
{
    ...
    [Networked]
    public int BlueScore_Minions { get; set; } // WARNING : 이거 0부터 시작함
    [Networked]
    public int RedScore_Minions { get; set; }

    [Networked]
    public PlayerRef masterRef { get; set; }
    ...
    public override void FixedUpdateNetwork() // GameState Sync는 첫 클라가 정보 관리 담당, 나머지가 구독. 필요시 RPC로 변경.
    {
        ...
        {
            masterRef = NetworkManager.Instance.runner.LocalPlayer;
    ...
    [Rpc (RpcSources.All, RpcTargets.StateAuthority)]
    public void RPC_SetRedGoodsCOunt(int _count)
    {
        RedScore_Goods = _count;
    }

    [Rpc (RpcSources.All, RpcTargets.StateAuthority)]
    public void RPC_SetBlueGoodsCOunt(int _count)
    {
        BlueScore_Goods = _count;
    }
